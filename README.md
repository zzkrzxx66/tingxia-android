...
- 后台播放、锁屏/蓝牙控制、断点续听、逐书倍速和逐书跳过片头片尾
- 桌面播放小组件，显示书名、章节和进度并支持上一章、播放/暂停、下一章
- 进程回收后从系统媒体中心恢复最近播放队列
- 睡眠定时、本章结束暂停、渐弱停止
- 书架搜索/排序/完成筛选，章节完成状态与批量标记
- 播放器内选集抽屉与书籍详情章节列表共用一套控件：搜章节/倒序/筛选/定位在听/选集跳转，长按多选批量缓存与标记
- 书籍信息、章节标题和书签备注管理
- 本地书一键同步在线信息（简介/作者/分类/字数/封面/章节标题），可一键清除还原
- 在线书：真人有声版本与 AI 朗读（TTS）音色任选，TTS 覆盖没有真人版的书，换音色保留进度
- 在线书追更：手动检查更新 + 打开应用时自动扫，新章只追加不动已有章节，带未读徽标
- 播放页两页左右滑动：封面页 + 正文页。AI 朗读书可句级声文同步（当前句高亮、自动跟随、点句子跳播），真人有声为普通阅读
- 在线找书：搜索翻页与历史、分区发现页、结果直接标注真人有声/AI 朗读、评分与收听人数
- 自定义封面与后台播放错误处理策略
