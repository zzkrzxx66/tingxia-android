...
- **修复桌面组件在 App 进程已被杀时点了没反应**。上一版的组件按钮只在 App 还活着时有效。
  - 点击广播确实到了，但代码在发出命令后立刻释放了 MediaController。冷启动时这个 controller
    是播放服务唯一的客户端（只被 bind、没有人 start），而 `play()` 触发的是
    `onPlaybackResumption`——要先读数据库把上次那本书恢复回来。等它读完，服务早已因为解绑被
    销毁，所以那一下等于没点。media3 只有在播放真正开始后才会把自己变成前台服务撑住。
  - 现在连接会在广播结束后继续保留一小段时间，跨过“请求播放 → 前台通知建立”这段窗口；
    连点两下也不用重连。
  - 冷启动时点上一章/下一章以前是彻底空操作（队列还是空的）：现在先恢复上次的书，等目录到位
    再执行跳章，然后按真实状态重绘组件。
  - 顺带：开机或添加组件时的刷新不再去连接会话，只用缓存快照重绘——以前它会白白拉起整个播放服务。
  - 一个改不了的边界：把 App 从任务列表划掉等于强制停止的 ROM（小米/华为一类），强停后系统不再
    投递任何广播，只有先打开一次 App 才能恢复。

## 0.17.1

- 正文从底部抽屉改成**播放页的第二页**，就像听歌看歌词那样：左右滑动在封面页与正文页
  之间切换，顶部两个小点标示当前页，右上角图标也能一键往返。
  - 正文页保留上一章/播放/下一章与时间，看字时不用为了暂停切回去。
  - 当前句以外的文字减淡，并把在读的那行停在屏幕上三分之一处（不是顶部）；
    手动滚动即交回控制权，右上角定位图标重新跟随。
  - 书籍详情里仍是抽屉（那里没有播放位置），点句子从那一刻开始播放；
    抽屉标题行与 A-/A+ 之前会重叠，现已分开。

## 0.17.0

- **声文同步（像歌词那样）**。上一版的“看正文”只是一堆不动的字，现在它会跟着声音走。
  - 上游本来就有句级时间轴（`/reading/reader/audio/timepoint/`），不需自己做强制对齐；
    服务端新增 `/audio/timeline` 把时间点与段落文本拼好，客户端只管渲染和高亮。
  - AI 朗读（TTS）的时间轴对得上小说正文，字符级严丝合缝：一章 172 句全部命中，
    末句结束时间等于章节时长。当前句高亮、列表自动跟随（手动滚动即取消跟随）、
    **点句子直接跳到那一刻**，书籍详情里点句子则从那一刻开始播放。
  - 高亮在两次轮询之间插值（播放位置 500ms 一报），否则每一句交界处都能看出滞后。
  - 真人有声不开句级高亮，这是数据决定的：它的时间点对的是旧一版 ASR 文稿
    （实测 15/15 章 `timepoint_item_version` 与 `item_version` 不一致，13/15 章段落数也不同，
    字符总长差约 4%），而上游没有按版本取文稿的接口。高亮错的句子比不高亮更差，
    所以服务端直接返回 `synced=false`，客户端退回普通阅读。
  - 顺带修掉上一版的一个弱点：真人有声现在用自己的文稿（同一个 itemId），不再按章号
    去猜小说的哪一章，“找不到对应正文”这个边界消失，连片头报幕都在文里。

### 服务端

- 新接口 `/audio/timeline/{bookId}/{itemId}`（段落 + 句级时间点 + `synced` 标志）与
  `/audio/timepoint/{bookId}/{itemId}`（原始上游响应）。
- `/chapter` 新增 `?raw=true` 可取带段落索引的原始 HTML；默认响应不变。
- fq 站点开启 JSON gzip：一章时间轴 64KB → 11KB。
